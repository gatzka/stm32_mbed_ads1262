...
{
	dma2_stream0->CR = DMA_SxCR_DIR_1 | DMA_SxCR_TCIE | DMA_SxCR_TEIE | DMA_SxCR_DMEIE | DMA_SxCR_PSIZE_1 | DMA_SxCR_MSIZE_1;
...
{
	static const uint32_t errors = DMA_LISR_FEIF0 | DMA_LISR_DMEIF0 | DMA_LISR_TEIF0;
	uint32_t lisr = dma2->LISR;

	if ((lisr & errors) == errors) {
		pc.printf("DMA transfer error: lisr: 0x%08x\n", lisr);
	} else {
		if ((lisr & DMA_LISR_TCIF0) == DMA_LISR_TCIF0) {
...
int main(void) {
	...
	NVIC_SetVector(DMA2_Stream0_IRQn, (uint32_t)&dma_irq_handler);
